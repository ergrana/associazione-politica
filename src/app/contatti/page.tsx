  ...
  return (
    <main className="min-h-screen">
      ...
      <section className="relative overflow-hidden">
        ...
        {/* overlay più morbido con gradiente */}
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 text-white">
          ...
          <p className="mt-4 max-w-2xl text-white/90">
            Stampa, segnalazioni o partnership: scrivici con un click o passa a trovarci in sede.
          </p>
        </div>
      </section>
      ...
      <section className="py-12 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
        <div className="grid md:grid-cols-2 gap-12 items-start">
          ...
          <div className="flex flex-col gap-10">
            <SectionTitle title="Recapiti diretti" />
            <RecapitiTimeline />

            <div className="border-t pt-8">
              <SectionTitle title="Seguici sui social" />
              <p className="text-slate-600">
                Resta aggiornato su iniziative, eventi e campagne.
              </p>
          ...
          <div className="flex flex-col gap-10">
            ...
            <div>
              <SectionTitle title="La nostra sede" />
      ...
      <section className="py-16 bg-slate-50">
        <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <div className="mt-6 flex justify-center gap-3">
            ...
            <a
              href="mailto:info@italianinelmondo.it?subject=Richiesta%20incontro&body=Ciao%2C%20vorrei%20fissare%20un%20appuntamento%20in%20sede.%0A%0ANome%3A%20%0ATelefono%3A%20%0ADisponibilit%C3%A0%3A%20%0A%0AGrazie!"
...
function SectionTitle({ title }: { title: string }) {
  return (
    <h3 className="text-lg font-semibold tracking-tight">
      <span className="inline-block h-5 w-1.5 rounded-full bg-indigo-600 align-middle mr-2" />
      {title}
    </h3>
  );
}

/** Timeline essenziale (senza riquadri) */
function RecapitiTimeline() {
  const items = [
    {
      k: "Email generale",
      desc: "Informazioni, richieste e segnalazioni.",
      action: { label: "info@italianinelmondo.it", href: "mailto:info@italianinelmondo.it" },
    },
    {
      k: "PEC",
      desc: "Comunicazioni formali e protocollate.",
      action: { label: "italianinelmondo@pec.it", href: "mailto:italianinelmondo@pec.it" },
    },
    {
      k: "Stampa",
      desc: "Ufficio comunicazione e media partner.",
      action: { label: "press@italianinelmondo.it", href: "mailto:press@italianinelmondo.it" },
    },
    {
      k: "Telefono",
    ...
    <ol className="relative mt-2">
      ...
      {items.map((it, i) => (
        <li key={i} className="relative pl-10 py-5">
          <span className="absolute left-0 top-6 -translate-y-1/2 inline-flex items-center justify-center w-6 h-6">
            <span className="w-2.5 h-2.5 rounded-full bg-indigo-600" />
          </span>

          <p className="font-semibold leading-6">{it.k}</p>
...
function Slideshow({ images }: { images: string[] }) {
  ...
  return (
    ...
    >
      <div className="relative h-72 sm:h-80">
        <Image src={images[i]} alt={`Sede — immagine ${i + 1}`} fill className="object-cover" />
      </div>
  ...
  return (
    <div className={`flex flex-wrap items-center gap-3 ${className}`}>
      <IconCircle
        href="#"
        label="Instagram"
        className="bg-gradient-to-tr from-fuchsia-500 to-amber-400"
      >
        <IconInstagram />
      </IconCircle>
      <IconCircle href="#" label="Facebook" className="bg-blue-600">
        <IconFacebook />
      </IconCircle>
      <IconCircle href="#" label="YouTube" className="bg-red-600">
        <IconYouTube />
      </IconCircle>
      <IconCircle href="#" label="Telegram" className="bg-sky-500">
        <IconTelegram />
      </IconCircle>
    </div>
  );
...
function IconCircle({
  ...
  return (
    <a
      ...
      aria-label={label}
      title={label}
      className={`inline-flex h-10 w-10 items-center justify-center rounded-full text-white shadow-sm ring-1 ring-black/5 hover:scale-105 transition ${className}`}
    >
